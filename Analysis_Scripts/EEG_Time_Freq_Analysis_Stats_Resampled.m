...
for iTimes = 1:12
   ...
   for iFreqs = 1:4 
       ...
       for iChans = 5;%1:4
           ...
           theseChans = [1,3; 4,6; 7,9; 10,15; 7,15];
           ...
           pValMat(iTimes,iFreqs) = var1.pValueANOVA;  %statOutput(4);
           fValMat(iTimes,iFreqs) = var1.fValObserved;
           
           % if doing multiple channels
...
for pairwiseComp=1:3
    subplot(1,3,pairwiseComp);
    for iChan=1%1:4
        %subplot(4,1,iChan);
        %imagesc(pValMatBinary(:,:,iChan,pairwiseComp)');
        imagesc(pValMatBinary(:,:,pairwiseComp)');
        set(gca,'YDir','normal');
        pbaspect([1,1,1])
        colormap jet
