...
pairwiseMat = pairwiseMat(351:551);

% plot resampled stat results
for iIter=1:length(pairwiseMat)
    
    
    
    
   if pairwiseMat(iIter)<.05
       line([theseTimes(iIter), theseTimes(iIter+1)],[-3 -3],'linewidth',10,'color','k')
...
saveas(h1,[destDirPlot '/' 'EEG_ERP_P3_Control.eps'],'epsc');
...
saveas(h2,[destDirPlot '/' 'EEG_ERP_P3_Topos_Control.eps'],'epsc');
