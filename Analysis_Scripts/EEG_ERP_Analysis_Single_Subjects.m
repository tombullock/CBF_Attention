...
destDirPlot = '/home/bullock/CBF_Attention/Plots/P3_Single_Subs';
...
for iChan=5;%1:4
    ...
    for iSub=1:12
        ...
        for i=1:4
            ...
            meanData = squeeze(mean(mean(ERP_Tar(iSub,i,theseChans,:),1),3)); % SMOOTHED FOR PLOTTING PURPOSES?
            semData = squeeze(std(mean(ERP_Tar(iSub,i,theseChans,:),3),0,1))./(sqrt(size(ERP_Tar,1)));
            ...
            titleText = ['sj' num2str(subjects(iSub)) ' nTrials:' num2str(ERP_Tar_nTrials(iSub,:))];
            
        end
        
        title(titleText);
        
        legend('Air','HCap','Hpo','Hpox','location','northwest')
        
        saveas(h,[destDirPlot '/' sprintf('sj%d_P3_ERP.jpg',subjects(iSub))],'jpeg');
        close
