...
for iTimes = 1:12
   ...
   for iFreqs = 1:4 
       ...
       for iChans = 1;%1:4
           ...
           theseFreqs = [1,3; 4,7; 9,12; 13,30];
           
           theseChans = 7:15;%[1,3; 4,6; 7,9; 10,15];
           
           % grab times, freqs and chans
           %observedData = mean(mean(mean(allERSP(:,:,theseChans(iChans,1):theseChans(iChans,2),find(freqs==iFreqs):find(freqs==iFreqs)+1,theseTimes(iTimes,1):theseTimes(iTimes,2)),3),4),5);
...
%               clear statOutput
          ...
          % tValsObs(5,:) = cohens_d;
           ...
           % if doing single subset of chans
           %pairwiseMat(iTimes,iFreqs,:) = tValsObs(3,:);
...
%           statOutput = teg_repeated_measures_ANOVA(observedData,[var1_levels],{var1_name});
          ...
          % [H,P,CI,STATS] = ttest(observedData(:,1),observedData(:,2));
           ...
           pValMat(iTimes,iFreqs) = tValsObs(4);
